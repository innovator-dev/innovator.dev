...
const app=(()=>({slide:function(){let c=document.querySelector(".btn-menu"),d=document.querySelector(".slide-menu");null!==c&&null!==d&&c.addEventListener("click",a=>{a.preventDefault();a.stopPropagation();a=d.querySelectorAll(".dialog-action");a.length&&a.forEach(e=>{let f=e.getAttribute("data-ref"),g=document.querySelector(f);f&&g&&g.childNodes[0]&&(d.innerHTML=d.innerHTML.replace(e.parentNode.outerHTML,g.childNodes[0].innerHTML))});const b=new Dialog(`<div class="container">${d.innerHTML}</div>`,
{dialogClassName:"slide-menu-dialog",dialogPlaceholderClassName:"slide-menu-dialog-placeholder",position:{x:0,y:0},callback:{onShow:()=>{c.style.visibility="hidden";document.body.classList.add("no-scroll");document.querySelector(".logo").classList.add("logo-slide-menu")},onClose:()=>{c.style.visibility="visible";document.body.classList.remove("no-scroll");document.querySelector(".logo").classList.remove("logo-slide-menu")}}});(a=b.dlg.querySelector("button.btn-slide-close"))&&a.addEventListener("click",
e=>{e.preventDefault();e.stopPropagation();b.close()})})},dialog:function(c,d=null){c=document.querySelectorAll(c);c.length&&c.forEach(a=>{let b=a.getAttribute("data-ref"),e=document.querySelector(b),f=a.getAttribute("data-attr");b&&e&&a.addEventListener("click",g=>{g.preventDefault();g.stopPropagation();f&&f.includes("closeAll")&&Dialog.closeAll();a.classList.add("selected");const h=new Dialog(e.innerHTML,{dialogClassName:f&&f.includes("isDropdown")?"dialog-dropdown":"dialog-shadowed",dialogPlaceholderClassName:f&&
f.includes("isDropdown")?"dialog-placeholder":"dialog-placeholder-shadowed",closeOnEsc:!0,closeOnOutsideClick:!0,linkTo:f&&f.includes("linkTo")?a:null,callback:{onClose:()=>{a.classList.remove("selected")}}});d&&d(h,a.dataset||null);(g=h.dlg.querySelector(".close-dialog"))&&g.addEventListener("click",k=>{k.preventDefault();k.stopPropagation();h.close()})})})}}))();(()=>{app.slide();app.dialog(".dialog-action")})();
